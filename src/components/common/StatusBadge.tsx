...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ 
  ...
  if (isAtRisk) {
    return (
      <span className={`inline-flex items-center rounded-full bg-red-100 text-red-800 border border-red-300 shadow-xs animate-pulse ${sizeClasses[size]} ${className}`}>
        <AlertTriangle className={size === 'sm' ? 'w-3 h-3 text-red-600' : 'w-3.5 h-3.5 text-red-600'} />
        <span>{t.statusAtRisk}</span>
      </span>
    );
  }
    ...
    if (priority === 'urgent') {
      return (
        <span className={`inline-flex items-center rounded-full bg-red-50 text-red-700 border border-red-200 ${sizeClasses[size]} ${className}`}>
          ...
          <span>{t.priorityUrgentBadge}</span>
        </span>
      );
    }
    return (
      <span className={`inline-flex items-center rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 ${sizeClasses[size]} ${className}`}>
        ...
        <span>{t.priorityRoutineBadge}</span>
      </span>
    );
  }
    ...
    case 'accepted':
      return (
        <span className={`inline-flex items-center rounded-full bg-amber-50 text-amber-800 border border-amber-300 ${sizeClasses[size]} ${className}`}>
          ...
          <span>{t.acceptedBadge}</span>
        </span>
      );
    case 'scheduled':
      return (
        <span className={`inline-flex items-center rounded-full bg-teal-50 text-teal-800 border border-teal-300 ${sizeClasses[size]} ${className}`}>
          <Calendar className={size === 'sm' ? 'w-3 h-3 text-teal-600' : 'w-3.5 h-3.5 text-teal-600'} />
          <span>📅 {t.statusScheduled}</span>
        </span>
      );
    case 'arrived':
      return (
