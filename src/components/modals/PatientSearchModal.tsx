...
export const PatientSearchModal: React.FC<PatientSearchModalProps> = ({ isOpen, onClose, onSelectPatient }) => {
  const { patients, referrals, selectReferral } = useReferralStore();
      ...
      <div className="bg-white rounded-2xl max-w-xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-3.5 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold">Fast Patient Registry Search</h3>
          </div>
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50">
          <input
            ...
            placeholder="Search by Patient Name, Phone, Village, or ABHA ID..."
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-2.5">
          {results.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-6">No matching patients found.</p>
          ) : (
            results.map(patient => {
                ...
                >
                  <div>
                    ...
                    <p className="text-slate-500 text-[11px] mt-0.5">
                      📍 {patient.village}, {patient.block} · Phone: {patient.phone}
                    </p>
                    {patient.abhaId && (
                    ...
                    {activeRef ? (
                      <div className="mt-1.5 inline-flex items-center gap-1.5 text-[10px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                        ...
                        Active Referral: #{activeRef.id} ({activeRef.status})
                      </div>
                    ) : (
