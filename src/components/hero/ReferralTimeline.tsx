...
interface StageConfig {
  ...
  sublabelKey: keyof typeof translations.en;
  icon: React.ElementType;
}

const STAGES: StageConfig[] = [
  { key: 'created', labelKey: 'stageCreated', sublabelKey: 'sublabelCreated', icon: Clock },
  { key: 'accepted', labelKey: 'stageAccepted', sublabelKey: 'sublabelAccepted', icon: Building2 },
  { key: 'arrived', labelKey: 'stageArrived', sublabelKey: 'sublabelArrived', icon: UserCheck },
  { key: 'consulted', labelKey: 'stageConsulted', sublabelKey: 'sublabelConsulted', icon: Stethoscope },
  { key: 'followup', labelKey: 'stageFollowup', sublabelKey: 'sublabelFollowup', icon: Calendar },
  { key: 'completed', labelKey: 'stageCompleted', sublabelKey: 'sublabelCompleted', icon: ShieldCheck }
];
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-xs font-bold px-2.5 py-1 rounded-lg bg-teal-900 text-white shadow-2xs">
              {t.careThreadPrefix}{referral.careThreadId || 'CT-1042'}
            </span>
            <span className="font-mono text-xs font-semibold px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200">
              {t.refPrefix}{referral.id}
            </span>
            {referral.parentReferralId && (
              <span className="text-[11px] font-medium text-slate-500 bg-slate-50 px-2 py-0.5 rounded border border-slate-200 flex items-center gap-1">
                ...
                {t.linkedFrom}{referral.parentReferralId}
              </span>
            )}
          </div>
      ...
      {referral.isAtRisk && (
        <div className="rounded-2xl border-2 border-red-300 bg-red-50/80 p-4 sm:p-5 shadow-xs space-y-3 animate-in fade-in slide-in-from-top-1 duration-150">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-white/90 p-3 rounded-xl border border-red-200 text-xs">
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500 block">{t.whoNeedsToAct}</span>
              <span className="font-bold text-slate-900">{referral.responsiblePerson || 'ASHA Worker'}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500 block">{t.suggestedActionLabel}</span>
              <span className="font-semibold text-teal-900">{referral.suggestedAction || t.contactPatient}</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            >
              ...
              <span>{t.callPatient} ({referral.patient.phone})</span>
            </a>
            {onRecordFailure && (
              <button
                onClick={onRecordFailure}
                className="px-3 py-1.5 bg-white hover:bg-red-50 text-red-800 border border-red-300 rounded-xl text-xs font-bold transition-colors cursor-pointer"
              >
                {t.updateDelayCause}
              </button>
            )}
            {onResolveFailure && (
              <button
                ...
                className="px-3 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold flex items-center gap-1 transition-colors shadow-xs cursor-pointer"
              >
                <Check className="w-3.5 h-3.5" />
                <span>{t.resolveIssue}</span>
              </button>
            )}
          </div>
        </div>
      ...
      <div className="pt-2">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
          {STAGES.map((stage, idx) => {
            ...
            const stageLabel = t[stage.labelKey] as string;
            const stageSublabel = t[stage.sublabelKey] as string;
              ...
              >
                ...
                <div className="mt-2">
                  ...
                  <p className="text-[10px] text-slate-500 truncate mt-0.5">
                    {matchedEvent ? matchedEvent.timestamp.split('·')[0] : stageSublabel}
                  </p>
                </div>
              </div>
            );
        ...
        <div className="pt-3 border-t border-slate-100 space-y-2">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center justify-between">
            ...
            <span className="text-[11px] font-normal text-slate-400 font-mono">
              {referral.events.length} {t.eventsLogged}
            </span>
          </h4>
